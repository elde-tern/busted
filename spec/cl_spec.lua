...
describe("Tests the busted command-line options", function()
  ...
  it("tests running with --dryrun specified", function ()
    local success, exitcode
    error_start()
    success, exitcode = execute("busted --pattern=cl_dryrun.lua$ --dryrun")
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    success, exitcode = execute("busted --pattern=before_each_dryrun.lua$ --dryrun")
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    success, exitcode = execute("busted --pattern=after_each_dryrun.lua$ --dryrun")
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    success, exitcode = execute("busted --pattern=setup_dryrun.lua$ --dryrun")
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    success, exitcode = execute("busted --pattern=teardown_dryrun.lua$ --dryrun")
    assert.is_true(success)
    assert.is_equal(0, exitcode)
    error_end()
  end)
